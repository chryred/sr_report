...
# 제외할 상태값
EXCLUDED_STATUSES = ["변경이관", "반려", "프로젝트이관", "작업이관"]
...
# 신 방식: 업무구분(FTE) → 문의유형 기본 매핑 (업무유형별 오버라이드가 없을 때 사용)
# ※ "일반 업무"는 FTE_GENERAL_TASK_INQUIRY_MAP / FTE_GENERAL_TASK_DEFAULT 로 처리
FTE_DIVISION_INQUIRY_MAP = {
    "데이터 작업":               "3. 데이터 추출 및 수정",
    "프로그램개선/개발":         "6. 신규개발 및 개선",
    "운영 업무":                 "5. 운영관리",
    "품질관리":                  "5. 운영관리",
    "프로젝트 지원/관리":        "5. 운영관리",
    "IT 운영 사업 기획 및 관리": "4. 운영지원",
    "정기 업무 지원":            "5. 운영관리",
}

# 신 방식: 업무구분(FTE) 내 업무유형(FTE)별 문의유형 오버라이드
# 업무구분 기본값(FTE_DIVISION_INQUIRY_MAP)보다 우선 적용
...
FTE_GENERAL_TASK_INQUIRY_MAP = {
    "문의 대응": "1. 기능문의",
    # 계정 및 권한 처리, 단순 처리, 오류 처리 → FTE_GENERAL_TASK_DEFAULT (2. 단순조치)
}
FTE_GENERAL_TASK_DEFAULT = "2. 단순조치"
